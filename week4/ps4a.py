...
def getWordScore(word, n):
    ...
    returns: int >= 0
    """
    return 50 + sum(SCRABBLE_LETTER_VALUES[i] for i in word) *len(word) if len(word) == n  else sum(SCRABBLE_LETTER_VALUES[i] for i in word) *len(word)
    ...
    while sum(hand.values()) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print(f'"{userInput}" earned {wordScore} points. Total: {total_score} points')
                # Update the hand
...
def playGame(wordList):
    ...
    """
    while True:
        SREGame = input("Enter n to deal new hand, r to replay last hand, or e to end game: ")
        if SREGame == "e":
            break
        if SREGame == "n":
